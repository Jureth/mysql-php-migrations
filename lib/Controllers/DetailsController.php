...
class DetailsController extends ActionController
{
    ...
    public function doAction()
    {
        ...
        $migration = MigrationHelper::getMigrationObject($migrationId);
        ...
        $filename = StringHelper::getFilenameFromTimestamp($migration->timestamp);
        $location = MPM::getMigrationsPath() . DIRECTORY_SEPARATOR . $filename;
        if (!file_exists($location)) {
            $this->_error = self::MIGRATION_FILE_MISSING;
            $this->_errorMessageReplacements[] = $filename;
            $this->_errorMessageReplacements[] = MPM::getMigrationsPath();
            $this->_printError();
            return;
        }
        ...
        $writer = CommandLineWriter::getInstance();
        ...
        $writer->addText("Migration DetailsController: $comment", 4);
        $writer->write();
    }
     ...
     *
     * @todo Move this up to the BaseController class so other controllers can
